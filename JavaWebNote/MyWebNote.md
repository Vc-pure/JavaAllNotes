...
1. ![image-20220719223027649](C:\Users\Nicolasbruno\AppData\Roaming\Typora\typora-user-images\image-20220719223027649.png)
...
- 你有没有注意一件事：当我的文件名设置为index.html的时候，不需要在web.xml文件中进行配置欢迎页面。这是为什么？
  ...
  - 实际上配置欢迎页面有两个地方可以配置：
    ...
    - Tomcat服务器的全局欢迎页面是：index.html   index.html   index.jsp。如果你一个web站点没有设置局部的欢迎页面(就是说如果没有自己手动在web.xml文件里设置欢迎页面），Tomcat服务器就会以index.html  index.html  index.jsp作为一个web站点的欢迎页面。
...
- 将一个Servlet类设置成欢迎页面的步骤：
  ...
  - 第二步：在web.xml文件下配置这个servlet
    ...
    - ```xml
      ...
      <servlet-mapping>
          <servlet-name>welcomeServlet</servlet-name>
          <url-pattern>/welcome/a/b/c/d/f</url-pattern>
      </servlet-mapping>
      ```

  - 第三步：在web.xml文件下配置欢迎页面

    - ```xml
      <welcome-file-list>
          <welcome-file>welcome/a/b/c/d/f</welcome-file>
      </welcome-file-list>
      ```

      - 注意：配置欢迎页面的时候不需要在开头加上“/”
...
- 在WEB-INF下目录下新建了一个文件：welcome.html
- 打开浏览器访问: http://localhost:8080/servlet07/WEB-INF/welcome.html出现了404错误
- 注意：放在WEB-INF目录下的资源是受保护的，在浏览器上不能够通过路径直接访问。所以像html、CSS、JS、image等静态资源一定要放在WEB-INF目录之外。
...
- 怎么获取前端浏览器用户提交的数据？
  ...
  - ```java
    Map<String,String[]> getParameterMap() 这个是获取Map								 (返回一个Map)
    Enumeration<String> getParameterNames() 这个是获取Map集合中所有的key				 (返回一个Enumeration)
    String[] getParameterValues(String name) 根据key获取Map集合的value					(返回一个String数组)
    String getParameter(String name)  获取value这个一维数组当中的第一个元素。这个方法最常用。(返回一个String)
    // 以上的4个方法，和获取用户提交的数据有关系。
        //牢记第三第四个方法
    ```
```
    
- 思考一个问题：如果是你，前端的form表单提交了一个数据之后，你准备怎么存储这些数据，你准备采用什么样的数据结构去存储这些数据呢？
  ...
  - 我会采用map集合来存储：
      ...
      - ```java
        Map<String,String>
            ...
            value存储String
            这种想法对吗？不对
            如果采用以上的数据结构存储，会发现key重复的时候value覆盖
            key				value
            ---------------------
            username		abc
            userpwd			111
...
```
      ...
      - 注意：前端表单提交数据的时候，假设提交了111这样的数字，其实是以字符串“111”的方式提交的，所以服务器获取到的一定是一个字符串的“111”，而不是一个数字。（前端提交的永远是字符串，后端获取到的也永远是字符串）



- 通过一个webapp（requesttest)的手工开发，来测试HttpServletRequest接口中的相关方法
...
- request对象实际上又称为”请求域“对象。
  ...
  - 应用域对象是什么？
    ...
    - 什么情况下会考虑向ServletContext应用域里绑定数据呢？
      - 第一：这个数据的数据量很小
      - 第二：这个数据是所有用户共享的数据
      - 第三：这个数据很少的进行修改操作
      - 在以上三个条件都满足的情况下，使用这个应用域对象，可以大大提高我们程序的执行效率
      - 实际上向应用域当中绑定数据，就相当于把数据放到了缓存（cache）当中，然后用户访问的时候直接从缓存中取，减少IO的操作，大大提升了系统的性能，所以缓存技术是提高系统性能的重要手段。

    - 你见过哪些缓存技术呢？

      - 字符串常量池
      - 整数型常量池   [-128~127],但凡是在这个范围当中的Integer对象，不再创建新对象，直接从整数型常量池中获取，大大提升系统性能
      - 数据库连接池（提前创建好N个连接对象，将连接对象放到集合当中，使用连接对象的时候，直接从缓存中拿。省去了连接对象的创建过程，效率提升）
      - 线程池（Tomcat服务器就是支持多线程的。所谓的线程池就是提前创建好N个线程对象，将线程对象存储到集合当中，然后用户请求过来之后，直接从线程池里获取线程对象，直接拿来用。提升系统性能）
      - 后期还会学习更多的缓存技术，比如：redis，mongoDB...

    - ServletContext当中有三个操作域的方法：

      - ```java
        void setAttribute(String name,Object obj);  //向域中绑定数据
        void setAttribute(String name,Object obj);  //根据name从域当中获取数据
        void removeAttribute(String name);          //将域当中绑定的数据移除
        
        //以上的操作类似于Map集合的操作
        Map<String,Object> map;
        map.put("name",Object);  					//向map集合中放数据：放key和value
        Object obj=map.get("name")  				//根据map集合的key获取value
        map.remove("name");      					//根据map集合的key删除key和value这个键值对
        ```
...
##### 请求域对象（request对象）也有的三个操作域的方法以及转发的方法
...
- 两个Servlet怎么共享数据？
  ...
  - 可以将数据放到request域（请求域）当中，然后AServlet转发到BServlet，保证AServlet和BServlet在同一次请求当中，这样就可以做到两个Servlet或者多个Servlet共享一份数据。

- 转发的下一个资源必须是一个Servlet吗？

  - 不一定，只要是Tomcat当中的合法资源，都是可以转发的。 例如：html...
...
- 关于Request对象中两个非常容易混淆的方法：
  ...
  - ```java
    ...
    //之前一定是要执行过:request.setAttribute("name",new Object())
    Object obj=request.getAttribute("name");
    
    //以上两个方法的区别是声明？
    //第一个方法：获取的是用户在浏览器上提交的数据
    //第二个方法：获取的是在请求域中绑定的数据
    ```

##### HttpServletRequest接口中的其他常用方法:(包含一些乱码情况的解决方案)
...
- 实现步骤：
            ...
            private static String password=bundle.getString("password");
                static{
                    ...
                    try {
                        ...
                        //OCP开闭原则：对扩展开放，对修改关闭。 （什么是符合OCP呢？在进行扩展的时候不需要修改java源					代码）
                       // Class.forName("com.mysql.jdbc.Driver"); //这样写就属于是写死了
                        Class.forName(driver);   //用bundle就可以直接写driver了
  ...
  - 第五步：实现第一个功能：查看部门列表
    ...
    - 我们应该怎么去实现一个功能呢？	
      ...
      - 建议：可以从后端往前端一步一步写，也可以从前端一步一步往后端写。都可以。但是千万要记住不要想起什么写什么。写代码的过程最好是程序的执行过程。也就是说：程序执行到哪里，就写到哪里。这样一个顺序流下来以后，基本不会出现什么错误、意外
      - 从哪里开始？
        - 假设从前端开始，那么一定是从用户点击按钮那里开始的
      
    - 第一：先修改前端页面的超链接，因为用户首先点击的就是这个超链接
    ...
    - 第四：在DeptListServlet类的doGet方法中连接数据库，查询所有部门，动态的展示部门列表页面
    
      - 分析list.html页面中哪部分是固定死的，哪部分是需要动态展示的
    
      - list.html页面中的内容所有的双引号要替换成单引号，因为out.print("")这里有一个双引号，容易冲突（可用EditPlus操作）
    
      - 现在写完这个功能以后，你会有一种感觉，感觉开发很繁琐，只使用servlet写代码太繁琐了
  ...
  - 第六步：查看部门详情
    ...
    - 建议：从前端一步一步往后端实现。首先要考虑的是，用户点击的是什么？用户点击的东西在哪里？
      ...
      - 一定要先找到用户点的”详情“在哪里。找了半天，终于在后端的java程序中找到了
        ...
        - 注意：修改路径以后，这个路径是需要加项目名的。 "/oa/dept/detail"
    ...
    - 编写一个类：DeptDetailServlet继承HttpSerlvet ,重写doGet方法
  ...
  - 第七步：删除部门
    ...
    - 怎么开始？从哪里开始？从前端页面开始，用户点击删除按钮的时候，应该提示用户是否删除。因为删除这个动作是比较危险的。任何系统在进行删除操作之前，是必须要提示用户的，因为这个删除的动作有可能是用户误操作。（在前端页面上写JS代码，来提示用户是否删除。）
      ...
      - 解决404的问题：
        ...
        - 编写DeptDelServlet类继承HttpServlet，重写doGet方法
...
### 1.22 前端F12和后台Debug模式的使用教学（调错方法）

- ​		**在 javaweb 视频 《 28-实现部门删除功能  》 前半小时有详细教学**



### 1.23 用转发和重定向在web应用中完成资源的跳转
...
- 代码上有什么区别？
  ...
  - 转发
    ...
    - ```java
      ...
      // 合并一行代码
      request.getRequestDispatcher("/dept/list").forward(request, response);
      //为什么forward里面要传request和response？通俗一点讲，就是要保证AServlet和BServlet里面的request和response是同一个，也就是要保证这两个参数是在同一个请求域当中，保证不论怎么转发都是同一次请求。
  ...
  - 重定向
    ...
    - ```java
      ...
      response.sendRedirect("/oa/dept/list")
          //这行代码会将请求路径"/oa/dept/list"发送给浏览器，
          //浏览器会自发的向服务器发送一次全新的请求："/oa/dept/list"
              /*
                response对象将这个路径："/servlet09/b"响应给浏览器了
                浏览器又自发的向服务器发 送了一次全新的请求：http://localhost:8080/serlvet09/b
                所以浏览器一共发送了两此次请求：
                  第一次请求：http://localhost:8080/serlvet09/a
                  第二次请求：http://localhost:8080/serlvet09/b
                最终浏览器上地址栏上显示的地址当然是最后那一次请求的地址。所以重定向会导致浏览器地址栏上的地址发生改变
              */
...
- 转发和重定向的本质区别？
  ...
  - 转发：是由WEB服务器来控制的。A资源跳转到B资源，这个跳转动作是Tomcat服务器内部完成的。  (服务端跳转)
  - 重定向：是浏览器完成的。具体跳转到哪个资源，是浏览器说了算。  (客户端跳转)
...
- 跳转的下一个资源有没有要求呢？必须是一个Servlet吗？
  ...
  - 转发会存在浏览器的刷新问题。
    -  使用转发的话在浏览器页面添加数据，如果添加成功一条之后，继续一直按刷新则数据库里会一直增加同一条数据，这就是浏览器的刷新问题
    - 使用重定向就不会发生这种问题
...
- 转发的理解图解：
  ...
  - ![image-20220725001607602](C:\Users\Nicolasbruno\AppData\Roaming\Typora\typora-user-images\image-20220725001607602.png)
- 重定向的理解图解：
  - ![image-20220725002223960](C:\Users\Nicolasbruno\AppData\Roaming\Typora\typora-user-images\image-20220725002223960.png)
  - 
...
### 1.26 使用模板方法设计模式解决类爆炸

​	**使用模板方法设计模式优化oa项目**

- 上面的注解解决了配置文件的问题。但是现在的oa项目仍然存在一个比较臃肿的问题。
  - 一个单表的CRUD，就写了6个Servlet。如果一个复杂的业务系统，这种开发方式，显然会导致类爆炸。（类的数量太大。）
...
- 回顾：什么是一次请求：用户在浏览器上点击了一下，然后到页面停下来 ，可以粗略认为是一次请求。请求对应的服务器端的java对象是：request

- 一个会话当中包含多次请求。（一次会话对应N次请求）

-  在java的servlet规范中，session对应的类名：HttpSession（javax.servlet.http.HttpSession)
...
- 为什么需要session对象来保存会话状态呢？
  ...
  - 只要B和S断开了，那么关闭浏览器整个动作，服务器知道吗？session什么时候在服务器里删除呢？
    - 不知道，服务器是不知道浏览器关闭的。 
    - 而且因为服务器不知道浏览器关闭，在浏览器关闭以后，由于**session超时机制**，对应的当前会话的session在tomcat服务器里面并不会立即销毁，而是在隔了一段时间没人访问以后，tomcat服务器就认为这个会话无效了，然后就会把这个session给销毁了。
...
- JSESSIONID=xxxxxxx 这个是以Cookie的形式保存在浏览器的内存中的。浏览器只要关闭，这个cookie就没有了。

- session列表是一个Map集合。这个Map集合的**key**存储的是**sessionid**，这个Map集合的**value**存储的是对应的**session对象**
...
- session对象的实现原理具体图解：
  ...
  ![image-20220726204827859](C:\Users\Nicolasbruno\AppData\Roaming\Typora\typora-user-images\image-20220726204827859.png)

  

**为什么关闭浏览器，会话结束？**

- 关闭浏览器之后，浏览器中保存的sessionid消失，下次重新打开浏览器之后，浏览器缓存中没有这个sessionid，自然找不到服务器中对应的session对象。session对象找不到，等同于会话结束。
...
- HttpSession session=request.getSession(false);
  ...
  - 从服务器中获取当前的session对象，如果获取不到session，则不会新建，返回一个null。
...
- 一种销毁：是手动销毁
  ...
  - 浏览器关闭的时候，服务器是不知道的，服务器无法监测到浏览器关闭了，所以session对象的销毁要依靠session超时机制。
  - 但也有一种可能，系统提供了安全退出，用户可以点击这个按钮，这样服务器就知道你退出了，然后服务器会自动销毁session对象
...
##### **Cookie禁用之后session对象还能找到吗？**

- cookie禁用是什么意思？服务器正常发送cookie给浏览器，但是浏览器不要了。拒收了。并不是服务器不发了
- 找不到了。每一次请求都会获取到新的session对象。
- cookie禁用了，session机制还能实现吗？
  - 可以。需要使用URL重写机制。
  - URL重写机制的格式：http://localhost:8080/servlet12/test/session;jsessionid=DC5AD484CFDA7B8ACDBF2CABB61474C7
...
- session的实现原理中，每一个session对象都会关联一个sessionid，例如：
  - JSESSIONID=322AA84A97948B76BFB777D3915FC3D5
  - 以上的这个键值对数据其实就是cookie对象
  - 对于session关联的从cookie来说，这个cookie是被保存在浏览器的“运行内存”当中
  - 只要浏览器不关闭，用户再次发送请求的时候，会自动将运行内存中的cookie发送给服务器
  - 例如，这个Cookie：JSESSIONID=322AA84A97948B76BFB777D3915FC3D5 就会再次发送给服务器
  - 服务器就是根据322AA84A97948B76BFB777D3915FC3D5 这个值来找到对应的session对象的。
  
- cookie怎么生成？cookie保存在什么地方？cookie有啥用？浏览器什么时候会发送cookie，发送哪些cookie给服务器？？？？

- cookie最终是保存在浏览器客户端上。
  - 可以保存在运行内存中。 （浏览器只要关闭，cookie就消失了）
  - 也可以保存在硬盘文件中。（永久保存）
  
- cookie有啥用呢？
  - cookie和session机制其实都是为了保存会话的状态
  - cookie是将会话的状态保存在浏览器客户端上。（cookie对象是存储在浏览器客户端上的）
  - session是将会话的状态保存在服务器端上。（session对象是存储在服务器上的）
  - 为什么要有cookie和session机制呢？因为HTTP协议是无状态 无连接协议
  
- cookie的经典案例：
  - 京东商城，在未登录的情况下，向购物车中放几件商品。然后关闭京东商城，再次打开浏览器，访问京东商城的时候，购物车中的商品还在，这是怎么做的？我没有登录，为什么购物车中还有商品呢？
    - 将购物车中的商品编号放到cookie当中，cookie保存在硬盘文件当中，这样即使关闭浏览器，硬盘上的cookie还在。下一次再打开京东商城的时候，查看购物车的时候，会自动读取本地硬盘中存储的cookie，拿到商品编号，动态展示购物车中的商品。
      - 京东存储购物车中商品的cookie可能是这样的：productIds=xxxxxx,yyyy,zzzz,kkkkk
      - 注意：cookie如果清除掉，购物车中的商品就消失了。
  - 126邮箱中有一个功能，十天内免登录
    - 这个功能也是需要cookie来实现的。
    - 怎么实现的呢？
      - 用户输入正确的用户名和密码，并且同时选择十天内免登录，登录成功后浏览器客户端会保存一个cookie，这个cookie中保存了用户名和密码等信息，这个cookie是保存在硬盘文件当中的，十天有效。在十天内用户再次访问126的时候，浏览器自动提交126关联的cookie给服务器，服务器接收到cookie之后，获取用户名和密码，验证，通过之后自动登录成功。
      - 怎么让cookie失效？
        - 十天过后自动失效
        - 或者改密码
        - 或者在客户端浏览器上清除cookie
  
- cookie机制和session其实都不属于java中的机制，实际上cookie机制和session机制都是HTTP协议的一部分。php开发中也有cookie和session机制，只要是你做web开发，不管是什么编程语言，cookie机制和session机制都是需要的。
...
- 关于cookie的有效时间
  ...
  - 没有设置cookie时间：默认保存在浏览器的运行内存中，浏览器关闭则cookie消失。
...
- 关于cookie的path，cookie关联的路径
  ...
  - 假设现在发送的请求路径是“http://localhost:8080/servlet13/cookie/generate”生成的cookie，如果cookie没有生成path，默认的path是什么？

    - 默认的path是：http://localhost:8080/servlet13/cookie这个路径以及这个路径下的子路径，cookie都会被发送到服务器
...
- 使用cookie实现十天内免登录功能
  ...
  - 修改前端页面
    ...
    - 用户选择了复选框：就表示要支持十天免登录
    - 用户没有选择复选框：表示用户不想使用十天免登录功能
  - 修改Servlet中的login方法
...
-  我的第一个JSP程序：
  ...
  - 在WEB-INF目录之外，web目录之内创建一个index.jsp文件，然后这个文件中没有任何内容。
  
- 将上面的项目部署之后，启动服务器，打开浏览器，访问以下地址：
  - http://localhost:8080/jsp/index.jsp 展现出来的是一片空白
  - 实际上访问以上的这个： index.jsp，底层执行的是： index_jsp.class这个java程序
  - 这个index.jsp会被tomcat翻译生成index_jsp.java文件，任何tomcat服务器又会将index_jsp.java编译生成index_jsp.class文件
  - 访问index.jsp，实际上执行的是index_jsp.class中的方法。
  
- JSP实际上就是一个Servlet
  - index.jsp访问的时候，会自动翻译生成index_jsp.java,会自动编译生成index_jsp.class,那么index.jsp这就是一个类
  - index_jsp类继承HttpJspBase，而HttpJspBase类继承的是HttpServlet。所以，index_jsp类就是一个Servlet类
  - jsp的生命周期和Servlet的生命周期完全相同。完全就是一个东西。没有任何区别
  - jsp和servlet一样，都是单例的（假单例）

- jsp文件第一次访问的时候是比较慢的，为什么？

  - 为什么大部分的运维人员在给客户演示项目的时候，为什么提前把所有的jsp文件先访问一遍？
...
- JSP是什么？
  ...
  - 每一个web容器（web服务器）都会内置一个JSP翻译引擎

- **对JSP进行错误调试的时候，还是要直接打开JSP文件对应的java文件，检查java代码**

  - **（开启tomcat的时候控制台有一个CATALINA_BASE后面的有地址，那个地址的后面的\work\Catalina\localhost\jsp\org\apache\jsp就是这个对应的java文件所在位置）**

- 开发JSP的最高境界：

  - 眼前是JSP代码，但是脑中呈现的是java代码

- JSP既然本质上是一个Servlet，那么JSP和Servlet到底有什么区别呢？

  - 职责不同
    - Servlet的职责是什么：收集数据  （Servlet的强项是逻辑处理，业务处理，如何连接数据库，获取/收集数据。）		
    - JSP的职责是什么：展示数据。（JSP的强项是做数据的展示）

- **JSP基础语法：**

  - 在jsp文件中直接编写文字，都会被自动翻译到哪里？
    - 翻译到servlet类的service方法的out.write("翻译到这里")，直接翻译到双引号里，被java程序当作普通字符串，打印输出到浏览器。
    - 在JSP中编写的HTML CSS JS代码，这些代码对于JSP来说只是一个普通的字符串。但是JSP把这个普通的字符串一旦输出到浏览器，浏览器就会对HTML CSS JS代码进行解释执行。展现一个效果
  - **JSP的page指令（这个指令后面详细说）：解决响应时的中文乱码问题：**
    - 通过page指令来响应的内容类型，在内容类型的最后面添加：charset=UTF-8
      - 具体代码实现：**<%page contentType="text/html;charset=UTF-8"%>**
        - 表示响应的内容类型是text/html，采用的字符集UTF-8
        - JSP中的导包方法：<%@page import ="java.util.List, java.util.ArrayList"%>
  - **怎么在JSP中编写Java程序**：
    - <%  java语句; %>
      - 在这个符号当中编写的被视为java程序，被翻译到servlet类的service方法内部
      -  这里要细心点，要去思考，在<% %>这个符号里面写java代码的时候，要时时刻刻记住你正在“方法体”当中写代码，方法体中可以写什么，不可以写什么，心里是否明白呢？
        - 在service方法当中编写的代码是有顺序的，方法体当中的代码要遵循自上而下的顺序依次逐行执行
        - service方法当中不能写静态代码块，不能定义成员变量。。。
        - 在同一个JSP当中<% %>这个符号可以出现多个。
    - <%! %>
      - 在这个符号当中编写的java程序会自动翻译到service方法之外
      - 这个语法很少有，为什么？
        - 不建议使用，因为在service方法外面写静态变量和实例变量，都会存在线程安全问题，因为JSP就是servlet，servlet是单例的，多线程并发的环境下，这个静态变量和实例变量一旦有修改操作，必然会存在线程安全问题。
    - JSP的输出语句
      - 怎么向浏览器上输出一个java变量。
  ...
  - **JSP基础语法总结：**
    ...
    - <%@page  contentType="text/html;charset=UTF-8" %>
      - page指令，通过contentType属性来设置响应的内容类型
  ...
  - 使用Servlet+JSP完成oa项目的改造
    ...
    - 将之前原型中的中的html文件，全部修改为jsp，然后在jsp文件头部添加page指令（指定contentType防止中文乱码），将所有的JSP直接拷贝到IDEA的web目录下
    ...
    - 思考一个问题：如果我只用JSP这一个技术，能不能开发web应用？
      ...
      - 当然可以使用JSP来完成所有的功能。因为JSP就是Servlet，在JSP的<%%>里面写的所有代码就是在service方法当中的，所以在<%%>当中完全可以编写JDBC代码，连接数据库，查询数据，也可以在这个方法 当中编写业务逻辑代码，处理业务，都是可以的，所以使用单独的JSP开发web应用完全没有问题。 
    - 虽然JSP一个技术就可以完成WEB应用，但是不建议，还是建议采用servlet+JSP的方式进行开发，这样就都能将各自的优点都发挥出来。JSP就是做数据展示，Servlet就是做数据收集。（JSP中编写的java代码越少越好）一定要职责分明。
  ...
  - JSP文件的扩展名必须是xxx.jsp吗？
    ...
    - xxx.jsp文件对于tomcat来说只是一个普通的文本文件，web容器会将xxx.jsp文件最终生成java程序，最终调用的是java对象相关的方法，真正执行的时候，和jsp文件就没有关系了。
    ...
    - 包名bean是什么意思？
      ...
      - java是一杯咖啡，咖啡又是由一粒一粒的咖啡豆研磨而成的。
      - 整个java程序中有很多bean的存在。由很多bean组成。javabean其实就起到了封装数据的作用。
...
- ​	当前的oa应用存在的问题：
  - 任何一个用户都可以访问这个系统，都可以对这个系统中的数据进行增删改这些危险的操作。我只想让合法的用户去使用这个系统，不合法的用户不能访问这个系统，怎么办？
    - 加一个登录功能，登录成功的可以访问该系统，登录失败的不能访问
  - 实现登录功能：
    - 步骤1：数据库当中添加一个用户表t_user
...
- JSP的指令	
  ...
  - 指令包括哪些呢？
    ...
    - include指令：包含指令，在JSP中完成静态包含。很少用（这里不讲）
    - taglib指令：引入标签库的指令。这个到JSTL标签库再学，现在先不管
    - page指令：目前重点学习page指令
  ...
  - 关于page指令当中都有哪些常用的属性呢？
    ...
    - ```jsp
      ...
      true表示启动JSP的内置对象session，表示一定启动session对象，没有session对象会创建，如果没有设置，默认值就是session="true"。
...
- JSP的九大内置对象（基本在前面都详细讲过用过）
  ...
  - javax.servlet.ServletConfig **config **                       （对应1.13  ServletConfig接口详解）
    ...
    - 三大功效：
      - 第一功效：从某个域中取数据
        - 从这四个域中取：
          ...
          - application
      - 第二功效：将取出的数据转成字符串
        - 如果是一个java对象，也会自动调用java对象的toString()方法将其转换成字符串
      - 第三功效：将字符串输出到浏览器
        - 和这个一样：<%=%>，将其输出到浏览器
...
- EL表达式的使用：
  ...
  - ```jsp
    ...
    <%--   使用EL表达式获取    --%>
    ${这个位置些什么???   这里写的一定是存储到域对象中的时候的name}
    要这样写：
    ${userObj}
    等同于java代码：<%=request.getAttribute("userObj") %>
    你不要这样写：${"userObj"} 如果这样写，得不到存的数据，只会被当作一个普通的字符串输出
  ...
  - EL表达式优先从小范围的域中读取数据(在没有指定范围的前提下)
  ...
  - 掌握使用EL表达式，怎么从Map集合中取数据：（11.jsp）

    - ${map.key}

  - 掌握使用EL表达式，怎么从数组和list集合中取数据：（12.jsp）

    - ${数组[0]}
    - ${数组[1]}
    - ${list[0]}

  - page指令当中有一个属性，可以忽略EL表达式
  ...
  - 通过EL表达式获取应用的根：（14.jsp)

    - ${pageContext.request.contextPath}

  - EL表达式中的其他隐式对象：（15.jsp）
    ...
    - initParam
      ...
      - ```jsp
        ...
        <%--用户在浏览器上提交数据：http://localhost:8080/jsp/15.jsp?username=jack--%>
        用户名：${param.username}
        ...
        <%--这里的.后面的东西取决于web.xml文件里的param-name --%>
...
- 使用JSTL标签库的步骤：
  ...
  - 第一步：引入JSTL标签库对应的jar包
    ...
    - 在IDEA当中怎么引入？
      - 在WEB-INF下新建lib目录，然后将jar包拷贝到lib目录当中，然后将其“Add lib...（添加为库）”
      - 一定是要和mysql的数据库驱动一样，都是放在WEB-INF/lib目录下的
      - 什么时候需要将jar包放到WEB-INF/lib目录下？如果这个jar包是tomcat服务器没有的
  ...
  - 第三步：在需要使用标签的位置使用即可。表示使用的是标签，底层实际上还是java程序
...
- 在前端HTML代码中，有一个标签，叫做base标签，这个标签可以设置整个网页的基础路径。
  ...
  - 在当前页面中，凡是路径没有以斜杠开始，都会自动将base中的路径添加到这些路径之前
  ...
  - 动态获取base的写法：
    ...
    -  注意：最后的/不能少，是用在其他地方的时候给加在最开始的地方的
    - 中间不能有空格
...
- 当前的OA项目存在什么缺陷呢？
  - DeptServlet、EmpServlet、OrderServlet。每一个Servlet都是处理自己相关的业务。在这些Servlet执行之前都是需要判断用户是否登录了。如果用户登录了，可以继续操作，如果没有登录，需要用户登录。这段判断用户是否登录的代码是固定的，并且在每一个Servlet类当中都需要编写，显然代码没有得到重复利用。包括每一个Servlet也要解决中文乱码问题，也有公共的代码。这些代码目前都是重复编写，并没有达到复用。怎么解决？
...
- 一个过滤器怎么写呢？
  - 第一步：编写一个Java类实现一个接口：javax.servlet.Filter ，并且实现这个接口当中的所有方法	
    -   init方法：在Filter对象第一次被创建之后调用，并且只调用一次
    -   doFilter方法：只要用户发送一次请求，则执行一次。发送N次请求，则执行N次。在这个方法中编写过滤规则
    -   destroy方法：在Filter对象被释放/销毁之前调用，并且只调用一次
  
  - 第二步：在web.xml文件里对Filter进行配置，这个配置和Servlet很像
...
- 使用@WebFilter()的时候，Filter的执行顺序是怎样的呢？
...
- Filter的生命周期？
  ...
  - 和Servlet对象的生命周期一致。
  - 唯一的区别：Filter在默认情况下，在服务器启动阶段就会实例化，Servlet不会。

- Filter过滤器这里有一个设计模式：

  - 责任链设计模式
  - 过滤器最大的优点：
    - 在程序编译阶段不会确定调用顺序，因为Filter的调用顺序是配置到web.xml文件中的，只要修改web.xml配置文件中的filter-mapping的顺序就可以调整Filter的执行顺序。显然Filter的执行顺序是在程序运行阶段动态组合的。那么，这种设计模式被称为责任链设计模式。
  - 责任链设计模式的最大的核心思想：
...
![image-20220729005144408](C:\Users\Nicolasbruno\AppData\Roaming\Typora\typora-user-images\image-20220729005144408.png)
...
- **Servlet规范中提供了哪些监听器？**
  ...
  - javax.servlet.http包下：
    ...
    - HttpSessionBindingListener   
      - 该监听器不需要使用@WebListener注解进行标注
      - 假设User类实现了该监听器（public void User implements HttpSessionBindingListener)，那么User对象在被放入session的时候触发bind事件。User对象从session中删除的时候，触发unbind事件。
...
- **个人总结监听器的作用：**
  ...
  - ServletContextListener、 ServletRequestListener、HttpSessionListener   
    - 这类监听器是用来监测域对象的创建和销毁的
  - ServletContextAttributeListener、ServletRequestAttributeListener 、HttpSessionAttributeListener 
    -    这类监听器是用来监测域中数据的存储和删除或者替换的
  - HttpSessionBindingListener 
    -   这类监听器用来监测特定的对象，只有实现了这个接口的对象，它往session对象里放或者解绑数据的时候都会触发这个事件
  - HttpSessionIdListener  （不常用）
    - 这个监听器用来监听sessionId的状态 ： 当sessionId发生改变的时候这个监听器当中的唯一一个方法就会被调用。
  - HttpSessionActivationListener（不常用）
    - 这个监听器用来监听session对象的钝化和活化的。
      - 钝化：session对象从内存存储到硬盘文件
      - 活化：从硬盘文件把session对象恢复到内存

- **实现一个监听器的步骤**：以ServletContextListener为例。

  - 第一步：编写一个类实现ServletContextListener为例
  ...
  - 第二步：在web.xml文件中对ServletContextListener进行配置，如下：
    ...
    - 当然第二步也可以不使用配置文件，也可以使用注解，例如：@WebListener
...
- 思考一个业务场景：
  ...
  - 编写一个功能，记录该网站实时的在线用户个数。
  - 我们可以通过服务器端有没有分配session对象判断，因为一个session对象代表一个用户。有一个session就代表有一个用户。如果采用这种逻辑去实现的话，session有多少个，在线用户就有多少个。这种方式的话：**HttpSessionListener**够用了。那么只要session对象新建，则count++，然后将count存储到ServletContext域当中，在页面展示在线人数即可。
  - 业务发生改变了：只统计登录的用户的在线数量，这个该怎么办？
    - session.setAttribute("user",userObj);
    - 用户登录的标准是什么？session中曾经存储过User类型的对象。那么这个时候可以让User类型的对象实现**HttpSessionBindingListener**监听器，只要User类型对象存储到session域中，则count++，然后将count++存储到ServletContext对象中。页面展示在线人数即可。
